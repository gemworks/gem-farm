...
describe('gem farm (no min staking / cooldown)', () => {
  ...
  describe('gem operations', () => {
    ...
    async function verifyClaimedRewards(
      ...
      const a = await verifyClaimedRewardByType(
        farmer1Identity,
        rewardA.publicKey,
        rewardADestination
      );
      const b = await verifyClaimedRewardByType(
        farmer1Identity,
        rewardB.publicKey,
        rewardBDestination
      );
    ...
    async function verifyClaimedRewardByType(
      identity: Keypair,
      rewardMint: PublicKey,
      ...
      // @ts-ignore
      assert(rewardDestAcc.amount.eq(farmerAcc.rewardA.paidOutReward));
