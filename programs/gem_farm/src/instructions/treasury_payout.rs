...
pub struct TreasuryPayout<'info> {
    ...
    #[account(mut, seeds = [b"treasury".as_ref(), farm.key().as_ref()], bump = bump)]
...
impl<'info> TreasuryPayout<'info> {
    fn payout_from_treasury(&self, bump: u8, lamports: u64) -> ProgramResult {
        invoke_signed(
            ...
            &[&[b"treasury".as_ref(), self.farm.key().as_ref(), &[bump]]],
        )
    }
}
